...
class HospitalCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_patient = new_test_user(
            cls.env,
            login=f"patient_{uuid.uuid4().hex[:6]}",
            groups="hr_hospital.group_patient",
        )

        cls.doctor = (
            cls.env["hr.hospital.doctor"]
            .sudo()
            .create(
                {
                    "first_name": "Alice",
